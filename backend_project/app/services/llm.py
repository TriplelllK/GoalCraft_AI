...
КРИТИЧЕСКИ ВАЖНО для использования RAG-контекста:
- Если предоставлены фрагменты документов (ВНД/стратегии/KPI), используй конкретные числа, метрики и приоритеты из них
- Привязывай цели к реальным KPI и показателям из предоставленных документов
- Ссылайся на конкретные стратегические приоритеты, упомянутые в документах
- НЕ выдумывай метрики, если они есть в документах — используй их
"""
